...
#==========================================#
#-----------------CONSUME------------------#
advancement revoke @s only bcsmp:brewing/consume_lava_bottle
tag @s[gamemode=!creative] add ConsumedItem
execute store success score @s ConsumeItem if predicate bcsmp:brewing/holding_lava_bottle_mainhand
...
scoreboard players set @s ConsumedItemType 2
#------------------------------------------#
