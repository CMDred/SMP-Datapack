...
data modify entity @e[tag=NameGet,limit=1] Item set from storage bcsmp:shops PriceItem
execute store result score #Count Temp run data get storage bcsmp:shops PriceItem.Count
execute unless data storage bcsmp:shops PriceItem.tag.display.Name run tellraw @s ["",{"text":"You do not have enough ","color":"gold"},{"selector":"@e[tag=NameGet,limit=1]","color":"red"},{"text":"!","color":"gold"}]
