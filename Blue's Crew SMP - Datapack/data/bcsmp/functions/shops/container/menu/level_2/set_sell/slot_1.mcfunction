...
execute unless data storage bcsmp:shops SetItem.tag.display.Name run tellraw @a[scores={ContainerItems=1..},sort=nearest,limit=1] ["",{"text":"You have set ","color":"gold"},{"selector":"@e[tag=NameGet,limit=1]","color":"red"},{"text":" x","color":"gray"},{"score":{"name":"#Count","objective": "Temp"},"color":"gray"},{"text":" as the sold item for the 1st slot!","color":"gold"}]
