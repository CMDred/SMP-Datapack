...
# @e[tag=Shop,sort=nearest,limit=1] should be the shop we want to check if the entity is inside

execute store result score #Size Temp run data get entity @e[tag=Shop,sort=nearest,limit=1] data.Size
scoreboard players operation #Size Temp /= #Constant2 Temp
scoreboard players operation #Size Temp += #BonusSize Temp

execute store result score #ShopX Temp run data get entity @e[tag=Shop,sort=nearest,limit=1] Pos[0]
...
execute store result score #ShopZ Temp run data get entity @e[tag=Shop,sort=nearest,limit=1] Pos[2]
...
execute if score #ThisX Temp >= #MinX Temp if score #ThisX Temp <= #MaxX Temp if score #ThisZ Temp >= #MinZ Temp if score #ThisZ Temp <= #MaxZ Temp run scoreboard players operation @s IsInShopID = @e[tag=Shop,sort=nearest,limit=1] ShopID
#------------------------------------------# 